...
public class SuggestorTest {
	...
	public void sanityCheck() {
		...
		for (Map.Entry<String, String> entry : wordsToCheck.entrySet()) {
			...
			if (result != null && result.size() >= 1) {
				Assert.assertFalse("Failed on word " + word + ". Lookup("
						+ word + ")=" + value, word.equals(value));
			} else
	...
	public void checkProfanityFilter() {
		...
		for (String badword : badWords) {
			...
			if (result != null && result.size() >= 1) {
				Assert.assertFalse(
						"Profanity filter failed on word " + badword,
						badword.equals(result.get(0).value));
			} else
				Assert.fail("Fail to find suggestion for " + badword);
