 ...
 * edits. Higher distances are not supported. Note that the fuzzy distance is
 * measured in "byte space" on the bytes returned by the {@link TokenStream}'s
 * {@link TermToBytesRefAttribute}, usually UTF8. By default the analyzed bytes
 * must be at least 3 {@link #DEFAULT_MIN_FUZZY_LENGTH} bytes before any edits
 * are considered. Furthermore, the first 1 {@link #DEFAULT_NON_FUZZY_PREFIX}
 * byte is not allowed to be edited. We allow up to 1 (@link #DEFAULT_MAX_EDITS}
 * edit. If {@link #unicodeAware} parameter in the constructor is set to true,
 ...
 *
 * <p>
 * NOTE: This suggester does not boost suggestions that required no edits over
 * suggestions that did require edits. This is a known limitation.
 *
 * <p>
...
public final class TRFuzzySuggester extends TRAnalyzingSuggester {
	 ...
	 * parameters in Unicode code points (actual letters) instead of bytes.
	 */
	 ...
	 */
	public static final int DEFAULT_MIN_FUZZY_LENGTH = 1;
	 ...
	 *            <code>true</code> if transpositions should be treated as a
	 *            primitive edit operation. If this is false, comparisons will
	 *            implement the classic Levenshtein algorithm.
	 * @param nonFuzzyPrefix
	 *            length of common (non-fuzzy) prefix (see default
	 *            {@link #DEFAULT_NON_FUZZY_PREFIX}
	 * @param minFuzzyLength
	 *            minimum length of lookup key before any edits are allowed (see
	 *            default {@link #DEFAULT_MIN_FUZZY_LENGTH})
	 * @param unicodeAware
	...
	Automaton toLevenshteinAutomata(Automaton automaton) {
		...
		for (IntsRef path : ref) {
			if (path.length <= nonFuzzyPrefix || path.length < minFuzzyLength) {
				subs[upto] = Automata.makeString(path.ints, path.offset,
						path.length);
				upto++;
			} else {
				int ints[] = new int[path.length - nonFuzzyPrefix];
				System.arraycopy(path.ints, path.offset + nonFuzzyPrefix, ints,
						0, ints.length);
				...
				LevenshteinAutomata lev = new LevenshteinAutomata(ints,
						...
						transpositions);
				subs[upto] = lev.toAutomaton(maxEdits, UnicodeUtil.newString(
						path.ints, path.offset, nonFuzzyPrefix));
				upto++;
			}
		}
		...
		if (subs.length == 0) {
			...
			// no synonyms or anything: just a single path through the
			// tokenstream
			return subs[0];
		} else {
			...
			// this only happens if you have multiple paths anyway (e.g.
			// synonyms)
